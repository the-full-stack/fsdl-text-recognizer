...
if [ $(id -u) == 0 ] ; then
    ...
    if [[ -z "$GIT_REPO" ]]; then
        GIT_REPO=lukas/ml-class
    fi

    if [[ -z "$GIT_BRANCH" ]]; then
        GIT_BRANCH=master
    fi
    dir="$(echo "$GIT_REPO" | cut -d'/' -f2)"

    if [ ! -d "/home/$NB_USER/$dir" ]; then
        echo "Cloning $GIT_REPO into /home/$NB_USER/$dir"
        git clone https://github.com/$GIT_REPO /home/$NB_USER/$dir
        cd /home/$NB_USER/$dir
        git config credential.helper store
        cd /home/$NB_USER
        chown -R $NB_USER /home/$NB_USER
    fi
    if [[ "$START_REDIS" == "1" || "$START_REDIS" == 'yes' ]]; then
