...
# TODO: Rename "line" to "word" throughout the file.
class IamDatasetW(Dataset):
    """
...
def _get_line_strings_from_xml_file(filename: str) -> List[str]:
    ...
    xml_line_elements = xml_root_element.findall('handwritten-part/line/word')
...
def _get_line_regions_from_xml_file(filename: str) -> List[Dict[str, int]]:
    ...
    xml_root_element = ElementTree.parse(filename).getroot()  # nosec
    xml_line_elements = xml_root_element.findall('handwritten-part/line/word')
    l = [_get_line_region_from_xml_element(el) for el in xml_line_elements]
    # for r in l:
    #     if r is None:
    #         print("File %s has an empty word component element"%filename)
...
def _get_line_region_from_xml_element(xml_line) -> Dict[str, int]:
    ...
    """
    components = xml_line.findall('cmp')
    if len(components) == 0:
        return None
    x1s = [int(el.attrib['x']) for el in components]
    y1s = [int(el.attrib['y']) for el in components]
    x2s = [int(el.attrib['x']) + int(el.attrib['width']) - 1 for el in components]
    y2s = [int(el.attrib['y']) + int(el.attrib['height']) - 1 for el in components]
    return {
        # Bounding box of one word
