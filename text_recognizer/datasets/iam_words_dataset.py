...
from text_recognizer.datasets.iam_dataset_w import IamDatasetW
from text_recognizer import util

INTERIM_DATA_DIRNAME = Dataset.data_dirname() / 'interim' / 'iam_words'
DEBUG_CROPS_DIRNAME = INTERIM_DATA_DIRNAME / 'debug_crops'
PROCESSED_DATA_DIRNAME = Dataset.data_dirname() / 'processed' / 'iam_words'
...
class IamWordsDataset(Dataset):
    """
    Paragraphs with words GT from the IAM dataset.
    """
    def __init__(self, subsample_fraction: float = None, image_dim: int = 512, num_classes: int = 3):
        """
        dataset_args is of the following form:
        {
            image_dim  : 512
            num_classes: 3  # 2 or 3. 2 implies binary classification (word / background),
                            # 3 implies word labels will alternate between 1 and 2
        }
        """
        self.iam_dataset = IamDatasetW()
        self.iam_dataset.load_or_generate_data()

        self.num_classes = num_classes
        self.input_shape = (image_dim, image_dim)
        self.output_shape = (image_dim, image_dim, num_classes)
    ...
    def load_or_generate_data(self):
        ...
        self.x, self.y, self.ids = _load_iam_paragraphs(self.num_classes)
    ...
    def _process_iam_paragraphs(self):
        ...
        for filename in self.iam_dataset.form_filenames:
            ...
            _crop_paragraph_image(filename, line_region, crop_dims, self.input_shape, self.num_classes)
...
def _crop_paragraph_image(filename, line_regions, crop_dims, final_dims, num_classes):
...
def _load_iam_paragraphs(num_classes):
    print('Loading IAM paragraph crops and words ground truth from image files...')
    images = []
    gt_images = []
    ids = []
    ...
    gt_images = to_categorical(np.array(gt_images), num_classes).astype(np.uint8)
